...
func (c *Commit) PrepareWrites(txn *mvcc.MvccTxn) (interface{}, error) {
	...
	if commitTs <= txn.StartTS {
		return nil, errors.New("commitTs must be greater than the transaction startTs")
	}
...
func commitKey(key []byte, commitTs uint64, txn *mvcc.MvccTxn, response interface{}) (interface{}, error) {
	...
	if lock == nil || lock.Ts != txn.StartTS {
		...
		currWrite, _, err := txn.RoTxn.CurrentWrite(key)
		if err != nil {
			return nil, err
		}
		...
		} else if currWrite.Kind == mvcc.WriteKindRollback {
			respValue := reflect.ValueOf(response)
			keyError := &kvrpcpb.KeyError{Abort: fmt.Sprintf("key %v rolled back", key)}
			reflect.Indirect(respValue).FieldByName("Error").Set(reflect.ValueOf(keyError))
		}
		return response, nil
	}
